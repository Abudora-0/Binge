CREATE DATABASE IF NOT EXISTS bingedb;
USE bingedb;
...
CREATE TABLE comment (
    ...
    CONSTRAINT CHK_Comment_Likes  CHECK (LikeCount >= 0)
);
...
CREATE VIEW vw_CreatorDashboard AS
SELECT
    c.Id                        AS CreatorId,
    c.ChannelName,
    c.TotalSubscribers,
...
CREATE VIEW vw_CreatorStats AS
SELECT
    c.Id,
    c.ChannelName,
    c.TotalSubscribers,
...
GROUP BY c.Id, c.ChannelName, c.TotalSubscribers, c.TotalViews, u.Email, u.Country, u.Status;


CREATE VIEW vw_TrendingVideos AS
SELECT
    v.Id,
    v.Title,
    v.Views,
...
GROUP BY v.Id, v.Title, v.Views, v.UploadDate, c.ChannelName, cat.Name
ORDER BY v.Views DESC;


CREATE VIEW vw_TopCategories AS
SELECT
    cat.Name,
    COUNT(DISTINCT v.Id)     AS TotalVideos,
...
CREATE VIEW vw_VideoEngagement AS
SELECT
    v.Id,
    v.Title,
    c.ChannelName,
...
CREATE VIEW vw_MonthlyWatchSummary AS
SELECT
...
CREATE PROCEDURE sp_GetCreatorStats(
    IN p_CreatorId INT
)
BEGIN
    SELECT
...
CREATE TRIGGER trg_NotifyOnUpload
...
CREATE TRIGGER trg_UpdateCreatorViews
